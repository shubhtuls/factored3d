    ...
    if ((color_scheme == RGB_COLOR_SCHEME) || (color_scheme == ALBEDO_COLOR_SCHEME)) {
      ...
      if (color_scheme == NODE_COLOR_SCHEME) {
        if (!node->Name()) LoadInteger(0);
            ...
            R3SceneNode *nodeJson = node;
            if(!strncmp(node->Name(), "Model#", 6)){nodeJson = node->Parent();}
            std::vector<std::string> elems =  split(nodeJson->Name(), '#');
            //std::cout << elems.size();
            int idx = 0;
	    if(elems.size() == 2){
            	std::vector<std::string> floor_id =  split(elems[1], '_');
            	//if(floor_id.size() >= 2){std::cout << nodeJson->Name() << '\n';}
            	//std::cout << floor_id.size() << ' ' << floor_id[0];
            	if(floor_id.size() >= 2){
            	    idx = (atoi(floor_id[0].c_str()) + 1)*10000 + (atoi(floor_id[1].c_str()) + 1);
            	}
	    }
            LoadInteger(idx);
